...
def FederatedRound(
    ...
    if not active: active=[np.random.randint(0,len(clients))]

    grads=[]
    for cid in active:
        local = copy.deepcopy(global_model)
        opt = torch.optim.SGD(local.parameters(), lr=task.lr)
        local.train()
    ...
    for p in global_model.parameters():
        n=p.numel()
        upd=torch.tensor(g[ptr:ptr+n], dtype=p.dtype).view_as(p)
        p.data -= task.lr*upd
        ptr += n
